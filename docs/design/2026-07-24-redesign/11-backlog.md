...
- `build-release.yml` 的 MSI、便携包、SBOM、许可证、Ed25519 模型目录和签名发布清单路径
  已在本地逐段执行通过；GitHub 上的正式 tag 工作流仍须在最终提交后首次完整运行。
- 版本基线已统一为 `0.1.0`，Windows 产物继续明确声明 Authenticode 未签名。
- 翻译器组已经进入档案 schema、工作台、运行时重放、托盘与热键路径；切换时取消旧执行，
  只重译仍可见文字，存储或热应用失败会回滚。
- 自动化证据和仍未完成的真机/云凭据门槛以
  `docs/testing/hardware-acceptance-2026-07-23.md` 为准；本节不把候选包等同于硬件验收。
