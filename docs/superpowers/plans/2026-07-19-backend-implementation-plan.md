...
- [ ] Define canonical signed release manifest fields, embedded Ed25519 trust root, dual-key rotation, anti-downgrade state and Authenticode publisher identity before updater work begins.
...
- Create: `src/InfiniTranseon.Contracts/Translation/provider-matrix.json`
...
- [ ] Freeze a first-release provider matrix with stable IDs, regions, auth, API versions, capabilities, streaming/context/glossary support and test strategy. Built-ins: DeepL, Google Cloud Translation, Azure AI Translator, Baidu Translate, Tencent Cloud TMT and Alibaba Cloud Machine Translation; OpenAI, Anthropic, Gemini, DeepSeek, Qwen/Model Studio and Baidu Qianfan; Azure AI Vision, Google Cloud Vision, Baidu OCR and Tencent Cloud OCR.
- [ ] Audit every frozen built-in provider against the declarative REST schema capability matrix (auth scheme, request signing, body/response formats, streaming) before writing adapter code. Ship providers whose protocol fits as bundled versioned declarative definitions with golden fixtures; hand-write C# adapters only where the protocol exceeds the schema (for example SSE-streaming LLMs and HMAC-signed cloud APIs such as Baidu/Tencent/Alibaba). Record fit-or-handwritten status per provider in `provider-matrix.json` so API changes update data, not code, wherever possible.
...
- Produces signed installer and portable ZIP with identical base capabilities.
...
- [ ] Treat GitHub metadata as untrusted. Verify the canonical manifest against the embedded/rotated Ed25519 trust root, then verify bound version, channel, architecture and artifact SHA-256; additionally verify installer Authenticode publisher and persisted anti-downgrade state.
